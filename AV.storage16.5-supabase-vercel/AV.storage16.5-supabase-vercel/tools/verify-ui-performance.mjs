...
assert.ok(!html.includes("--av-card: var(--av-bg)"), "cards should not be flattened into the page background");
...
assert.ok(!html.includes('#main-content div[class*="bg-white"]'), "main content should not flatten every div background globally");
assertIncludes(html, "background: linear-gradient(180deg, var(--av-bg)", "main content should use a refined background field");
