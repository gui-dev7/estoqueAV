...
assertIncludes(html, "--av-card: var(--av-bg)", "div surfaces should match background");
...
assert.ok(/#main-content\s+div\[class\*="bg-white"\][\s\S]*?background-color:\s*var\(--av-bg\)/s.test(html), "main content divs should match the background");
